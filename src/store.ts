  ...
  try {
    ...
    const result = await callImageApi({
      ...
      onStreamProgress: (info) => {
        // 流式进度：立即传预览 URL 给 UI，绕过缩略图异步生成
        updateTaskInStore(taskId, {
          streamProgress: info.progress,
          streamPreviewUrl: info.previewUrl,
          ...(info.imageFormat ? { imageSourceFormat: info.imageFormat } : {}),
        })
        if (info.previewUrl) {
          const url = info.previewUrl
          void (async () => {
            const imgId = await storeImage(url, 'generated')
            cacheImage(imgId, url)
            updateTaskInStore(taskId, {
              outputImages: [imgId],
              streamProgress: info.progress,
            })
