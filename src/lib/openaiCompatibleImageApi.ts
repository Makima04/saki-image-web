...
/** 根据图片参数估算响应体总字节数，用于流式传输进度条 */
function estimateResponseBodySize(params: TaskParams): number {
  const [w, h] = (params.size || '1024x1024').split('x').map(Number)
  const pixels = (w || 1024) * (h || 1024)
  ...
  switch (params.output_format) {
    case 'png':
      ...
      break
    case 'jpeg':
      compressionRatio = { low: 0.03, medium: 0.06, high: 0.10, auto: 0.12 }[params.quality] || 0.12
      break
    case 'webp':
      compressionRatio = { low: 0.025, medium: 0.05, high: 0.08, auto: 0.10 }[params.quality] || 0.10
      break
    default:
      compressionRatio = 0.35
  }

  const base64BytesPerImage = rawBytes * compressionRatio * 1.37 + 100
  return Math.round(base64BytesPerImage * n * 1.05 + 2048)
}
  ...
  while (true) {
    ...
    for (const block of parts) {
      ...
      try {
        ...
        if (type) onEvent({ type, data: json as Record<string, unknown> })
      } catch { /* skip malformed events */ }
    }
  }
}

/** 从 Responses API SSE 流中提取图像结果 */
async function consumeStreamedResponses(
  response: Response,
  mime: string,
  params: TaskParams,
  onProgress?: (info: { type: string; progress: number; previewUrl?: string; imageFormat?: 'base64' | 'url' }) => void,
): Promise<CallApiResult> {
  const reader = response.body?.getReader()
  if (!reader) throw new Error('流式响应不支持')

  const eventLog: Array<{ type: string; raw: string }> = []
  const outputItems: Array<Record<string, unknown>> = []
  let finalPayload: ResponsesApiResponse | null = null
  const previewUrls: string[] = []
  const estimatedTotal = estimateResponseBodySize(params)

  await parseSSEStream(reader, (event) => {
    eventLog.push({ type: event.type, raw: JSON.stringify(event.data) })
    ...
    if (
      ...
      if (item && item.type === 'image_generation_call') {
        ...
        // 图片结果已到达 → 立即生成预览，检测数据格式
        const result = item.result as string | undefined
        if (event.type === 'response.output_item.done' && result && result.trim()) {
          const previewUrl = normalizeBase64Image(result, mime)
          previewUrls.push(previewUrl)
          const isUrl = isHttpUrl(result) || isDataUrl(result)
          onProgress?.({ type: event.type, progress: 85, previewUrl, imageFormat: isUrl ? 'url' : 'base64' })
        }
      }
    }

    if (event.type === 'response.completed') {
      const resp = (event.data.response || event.data) as ResponsesApiResponse
      if (resp.output && Array.isArray(resp.output) && resp.output.length > 0) {
        finalPayload = resp
      }
    }
  }, (bytesRead) => {
    const progress = Math.min(Math.round((bytesRead / estimatedTotal) * 100), 99)
    onProgress?.({ type: 'bytes', progress })
  })

  // 无完整 response.completed 数据时回退到累积项
  if (!finalPayload && outputItems.length > 0) {
    finalPayload = {
  ...
  if (
    ...
    finalPayload.output.length === 0
  ) {
    const err = new Error('接口未返回图片数据')
    ;(err as any).rawResponsePayload = JSON.stringify(
      finalPayload || eventLog,
      null,
      2,
    )
    throw err
  ...
  try {
    ...
    return {
      ...
      actualParamsList: imageResults.map((r) =>
        mergeActualParams(r.actualParams ?? {}),
      ),
      revisedPrompts: imageResults.map((r) => r.revisedPrompt),
    }
  } catch (e) {
    const err = e instanceof Error ? e : new Error(String(e))
    if (!(err as any).rawResponsePayload) {
      ;(err as any).rawResponsePayload = JSON.stringify(finalPayload, null, 2)
    }
    throw err
  }
}
...
async function callResponsesImageApiSingle(opts: CallApiOptions, profile: ApiProfile): Promise<CallApiResult> {
  ...
  try {
    ...
    return consumeStreamedResponses(response, mime, params, opts.onStreamProgress)
  } finally {
    clearTimeout(timeoutId)
  }
}
