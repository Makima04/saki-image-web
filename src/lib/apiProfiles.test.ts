...
describe('mergeImportedSettings', () => {
  ...
  it('skips imported profiles that already exist in current customized settings', () => {
    ...
    expect(merged.profiles).toHaveLength(2)
    expect(merged.profiles.find((p: any) => p.apiKey === 'current-key')).toMatchObject({ apiKey: 'current-key', model: 'current-model' })
    expect(merged.profiles.find((p: any) => p.apiKey === 'custom-key')).toMatchObject({ provider: 'kefu-xiang', apiKey: 'custom-key', model: DEFAULT_IMAGES_MODEL })
  })
  ...
  it('does not replace existing custom providers when only the default profile remains', () => {
    ...
    expect(merged.customProviders.map((provider) => provider.id)).toEqual(['custom-existing', 'kefu-xiang', 'custom-imported'])
    expect(merged.profiles).toHaveLength(2)
  })
  ...
  it('appends imported custom providers and keeps imported custom profile references', () => {
    ...
    expect(merged.customProviders.find((p: any) => p.id === 'custom-json')).toMatchObject({ id: 'custom-json', name: 'Custom JSON' })
    expect(merged.profiles).toHaveLength(2)
    expect(merged.profiles[1]).toMatchObject({
      name: 'Imported Custom',
      provider: 'custom-json',
...
\`\`\``)
  ...
  it('switches to custom provider preserving settings', () => {
    const provider = importCustomProviderDefinitionFromJson(JSON.stringify({
